...
export function AddProjectFlow({ request, onClose }: AddProjectFlowProps) {
  ...
  const openAddedProject = useCallback(
      ...
      try {
        ...
        if (result.ok) {
          if (result.project) {
            ...
            // Existing git worktree opened directly as a workspace; the
            // useOpenProject hook already navigated to it.
            onClose();
          }
          return;
        }
    ...
    [hostId, onClose, openNewWorkspaceForProject, openProject],
  );
      ...
      try {
        ...
        setState((current) =>
          setPageStatus(current, "github-location", {
            isSubmitting: false,
            error: (result.ok ? null : result.error) ?? "Unable to clone repository",
          }),
        );
      } catch (error) {
        setState((current) =>
